...
## 工作模式
- **加新工具**：對 Codex 說「我想做一個 XXX 工具」→ Codex 會建 `tools/<工具名>/` 子資料夾、引導我跟著 EP10 影片做
- **結束工作**：對 Codex 說「**收工**」→ 自動 commit + push + 更新 Obsidian 工作筆記
...
- **Cozy Rain Piano** `tools/cozy-rain-piano/` — 全自動 YouTube 睡眠音樂 pipeline：CC0 圖像 + Ken Burns ffmpeg + Codex SEO + YouTube API 每日自動上傳；英文市場關鍵字 `Cozy Rain Piano`

## 設計工具（open-design）
- **啟動**：對 Codex 說「開設計工具」（Mac 上直接用 terminal 啟動）
- **主程式路徑**：`~/Library/CloudStorage/GoogleDrive-tinghan@gmail.com/我的雲端硬碟/open-design/`（不在 git 管理範圍內）
- **何時用**：
  - 做新工具前先產出 UI 設計稿
  - 做海報、Landing Page、社群圖卡（如金門義診系列）
  - 需要快速生成多個視覺方向供選擇
- **代理**：自動偵測到你的 Codex，直接使用，不需額外設定
- **更新**：`cd ~/Library/CloudStorage/GoogleDrive-tinghan@gmail.com/我的雲端硬碟/open-design && git pull && pnpm install`
